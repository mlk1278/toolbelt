...
The controller passes the implementation report and review-package path to the reviewer, which writes its findings to a review file and returns a short verdict; the implementer fixes from that file. The controller reads return messages only, never reports, reviews, diffs, or logs.
